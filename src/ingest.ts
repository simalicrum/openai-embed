...
const program = new Command()
  .option(
    ...
    "Text file(s) to ingest into GPT embeddings vectorstore (accepts wildcards)."
  )
  .requiredOption(
    "-d, --data-store <dir>",
    "Directory to store vectorstore data."
  )
  .requiredOption(
    "-t, --input-type <type>",
    "Input type. Options are 'text', 'markdown' or 'pdf'."
  )
...
for await (const line of files) {
  ...
  if (options.newVectorStore) {
    const answer = await prompt({
      ...
      name: "question",
      message:
        "Creating a new vectorsotre will overwrite any data in an existing directory. Are you sure?",
    });
    if (!answer) {
      process.exit(0);
    }
  ...
  spinner.succeed(
    `Ingested ${chalk.green(
      docs.length
    )} documents into vectorstore ${chalk.green(options.dataStore)}.`
  );
}
